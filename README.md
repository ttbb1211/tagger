...
| 歌词 .lrc | 歌词页可勾选「同时导出 .lrc」把歌词另存为独立文件；**整轨 CUE 虚拟轨道自动强制导出**（虚拟轨道没有独立音频文件，.lrc 是唯一能持久化的方式）（本 fork，v1.4.4 起） |
...
- **歌词可导出 `.lrc`**（v1.4.4）：歌词页新增「同时导出 .lrc 歌词文件」勾选框——普通曲目可与内嵌歌词并存，写成同目录同名的 `.lrc`；**整轨 CUE 虚拟轨道强制导出**（虚拟轨道没有独立音频文件，内嵌无从写起，`.lrc` 是唯一能持久化的方式，路径为 `<父音频>.<轨号>.lrc`）。歌词清空且已存在 `.lrc` 时会一并删除该文件。
...
上述修复计划向上游提交 PR。**Windows/macOS/Linux 用户可直接从 [Releases](https://github.com/ttbb1211/tagger/releases) 下载安装包与各架构二进制**（当前 v1.4.4）。注意：下文「快速开始」中的 Docker 镜像名仍指向上游 `ghcr.io/ericwyn/tagger`（fork 未改），自建镜像请直接用本仓库源码 `docker build`。
