...
| 歌词 .lrc | 歌词页可勾选「导出 .lrc」把歌词另存为独立文件（普通曲目是「内嵌之外再存一份」，整轨 CUE 虚拟轨道则由你决定要不要歌词）（本 fork，v1.4.4 起） |
...
- **歌词可导出 `.lrc`**（v1.4.4）：歌词页新增「导出 .lrc 歌词文件」勾选框——普通曲目可与内嵌歌词并存，写成同目录同名的 `.lrc`；**整轨 CUE 虚拟轨道可选**（虚拟轨道没有独立音频文件、歌词无法内嵌，勾选后写成 `<父音频>.<轨号>.lrc`；不勾选则歌词只保留在曲库索引中，完整重扫会丢失）。歌词清空且已存在 `.lrc` 时会一并删除该文件。
